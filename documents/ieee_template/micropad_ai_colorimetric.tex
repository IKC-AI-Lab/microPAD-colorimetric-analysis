% Overleaf-ready IEEE conference paper for microPAD colorimetric analysis
% Uses local IEEEtran.cls in this folder when present.
\IfFileExists{documents/ieee_template/IEEEtran.cls}{%
  \makeatletter\def\input@path{{documents/ieee_template/}}\makeatother
}{}
...
The dataset is structured to capture real-world variability by combining four smartphone models---iPhone~11, iPhone~15, Realme~C55, and Samsung~A75---across seven controlled lighting configurations. Each microPAD instance yields seven images corresponding to distinct lamp combinations ranging from single-source to all-lamps-on conditions. Stage~1 raw photographs reside in \texttt{1\_dataset/\{phone\_model\}/} directories, Stage~2 cropping isolates individual strips, Stage~3 extracts rectangular concentration zones, and Stage~4 generates the three elliptical regions per zone. Consistent coordinate files accompany each stage to enable exact regeneration of processed imagery. \textbf{TODO-USER: Document camera-to-sample distance, incidence angle, white balance settings, and tripod or fixture details.} \textbf{TODO-USER: Provide information about ambient temperature, humidity control, and repeatability across acquisition sessions.}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{stage1_original_image.jpeg}
  \caption{Representative Stage\,1 raw capture from smartphone.}
  \label{fig:stage1}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{stage2_micropad_paper.jpeg}
  \caption{Stage\,2 crop isolating a single microPAD paper strip.}
  \label{fig:stage2}
\end{figure}

\subsection{Dataset Organization and Naming}
The processing pipeline writes sequential outputs to stage folders: \texttt{1\_dataset/} \textrightarrow{} \texttt{2\_micropad\_papers/} \textrightarrow{} \texttt{3\_concentration\_rectangles/} \textrightarrow{} \texttt{4\_elliptical\_regions/} \textrightarrow{} \texttt{5\_extract\_features/}. For Stage~4 patches, filenames follow \texttt{\{base\}\_con\{N\}\_rep\{M\}.ext} where \texttt{rep\{M\}} with $M\in\{0,1,2\}$ denotes the three replicates per concentration zone. A coordinate log (\texttt{coordinates.txt}) is generated atomically at each stage to preserve reproducibility across reruns. \textbf{CAUTION:} Do not edit \texttt{coordinates.txt} manually; if corrupted, delete and rerun the corresponding stage.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{stage3_concentration_rectangle.jpeg}
  \caption{Stage\,3 extraction of a concentration rectangle.}
  \label{fig:stage3}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\linewidth]{stage4_elliptical_region_1.jpeg}
  \caption{Stage\,4 example elliptical measurement region.}
  \label{fig:stage4}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{augmented_dataset_1.jpg}
  \caption{Example augmented dataset visualization used for model robustness.}
  \label{fig:augment}
\end{figure}
...
\subsection{White Reference and Normalization}
If a white reference region is used to normalize illumination effects, document the extraction method and pixel selection criteria. An example overlay is shown in Fig.~\ref{fig:white}.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{white_referenced_pixels_on_rectangle.png}
  \caption{Illustration of white-referenced pixel selection on a concentration rectangle.}
  \label{fig:white}
\end{figure}
